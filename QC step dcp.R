...
QC<-read.csv("C:/Users/22064705/OneDrive - UWA/My Documents/TERN LAI/250803_final_check/TERN_dcp_LAI_QC.csv")
...
for(i in 1:nrow(QC)){
  ...
  if(!is.na(img_thd)){
    outputs <- as.data.frame(getGap(img.bw,path_id=Code,export.image=F,th=img_thd))
  }else{
    outputs <- data.frame(path_id=Code,id='Blue',FC=NA,CC=NA,CP=NA,PAIe=0,PAI=0, CI=NA,k=0.5,imgchannel='Blue',gap_method='Macfarlane', gap_thd=NA, img_method='No plant material',img_thd=NA)
  }
  ...
  if(i%%50==0){
    write.csv(QC,'D:/TERN_dcp_LAI_QC_complete.csv',na='',row.names = F)
  }
}
