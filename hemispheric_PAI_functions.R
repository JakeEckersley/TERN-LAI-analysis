...
read_B_and_gamma_correct<-function(dhp,gamma=2.2,rgb=T,stretch_im=F){
  imgB <- terra::rast(dhp)
  if(nlyr(imgB)==1){
...
rgb_plot<-function(dhp_rast=rast_im){
  if("INT1U"%in%datatype(dhp_rast)){
    maxcolval<-255
  }else{
    maxcolval <- global(dhp_rast, fun = "max", na.rm = TRUE)$max
  }
...
hist_plot<-function(dhp_rast,thresh){
  dhp_rast<-as.data.frame(values(dhp_rast))
  # basic histogram
...
getGap<-function(img.bw,path_id=Code,thd=0.013,k=0.5,export.image=F){
  ...
  if(!'Small_gap'%in%colnames(tbf.sum)){
    tbf.sum$Small_gap<-0
  }
  ...
  if (export.image==TRUE){
    ...
    #terra::set.crs(canopy.rst, terra::crs(img.bw))
    terra::set.names(canopy.rst, base::names(yr))
    terra::set.ext(canopy.rst, ext)
    # terra::plot(canopy.rst)
    
    gg3.2<-ggplot(data = as.data.frame(canopy.rst,xy=T), aes(x = x, y = y,fill=Blue))+
      geom_raster()+
      scale_fill_gradient(low = "black", high = "white")+
      theme_bw()+
      theme(line = element_blank(),
